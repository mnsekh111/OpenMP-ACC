...
void run_sim(double *u, double *u0, double *u1, double *pebbles, int n,
	...
	while (1) {
		...
		for (i = 0; i < n; i++) {
//#pragma omp parallel for schedule(dynamic) default(shared) num_threads(nthreads)
#pragma omp parallel for schedule(static) default(shared) num_threads(nthreads)
			for (j = 0; j < n; j++) {
				//printf("%d",j);
