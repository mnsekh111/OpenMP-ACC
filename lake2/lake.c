 ...
 *******************************/
void
run_sim (double *u, double *u0, double *u1, double *pebbles, int n, double h,
	 double end_time, int nthreads)
{
      ...
      for (i = 0; i < n; i++)
	{
	  ...
	  for (j = 0; j < n; j++)
	      ...
	      else
		{
		  ...
		  un[i][j] = 2 * uc[i][j] - uo[i][j]
		      + VSQR * (dt * dt)
			  * ((uc[i][j - 1] + uc[i][j + 1] + uc[i + 1][j]
			      + uc[i - 1][j]
			      + 0.25
				  * (uc[i - 1][j - 1] + uc[i + 1][j - 1]
				      + uc[i - 1][j + 1] + uc[i + 1][j + 1])
...
void
init_pebbles (double *p, int pn, int n)
{
  int i, j, k, idx;
  int sz;
...
int
tpdt (double *t, double dt, double tf)
{
  if ((*t) + dt > tf)
    return 0;
  (*t) = (*t) + dt;
...
void
init (double *u, double *pebbles, int n)
{
  int i, j, idx;
 ...
 ********************************/
void
error_u (double *uerr, double *avgerr, double *ua, double *ub, int n)
{
  int i, j, idx;
 ...
 ********************************/
void
print_heatmap (char *filename, double *u, int n, double h)
{
